...
- **D-03 (revised 2026-05-29):** Subject template uses Supabase's native `{{ .Token }}` variable: `Acceso a tu viaje · {{ .Token }}` — this guarantees uniqueness per request (defeats Gmail threading) without requiring a custom `admin.generateLink()` + Resend SDK route handler. The token fragment is opaque to humans, so to preserve "which link is freshest" discoverability, the email **body** includes a line `Solicitado a las HH:mm` rendered server-side via `Intl.DateTimeFormat('es-MX', { hour: '2-digit', minute: '2-digit', hour12: false })`. Trade-off: uniqueness lives in the subject (Gmail-threading defense), readability lives in the body (user-facing freshness signal). Original `HH:mm`-in-subject approach rejected because it would require taking ownership of the entire mail-sending pipeline for Phase 1 — out of scope.
...
- **D-08 (revised 2026-05-29):** Brand mark is **wordmark only** for Phase 1: the literal string `SharedTrip` rendered in `--color-primary` (coral), Inter italic, **weight 700**. (Originally specified weight 600; revised to 700 to align with the UI-SPEC typography rule that only weights 400 and 700 are permitted across the design system — keeps a single source of truth for weights.) No SVG glyph yet. The PWA icon (manifest) uses a coral square with white `S` until a real mark is commissioned in Phase 5.
